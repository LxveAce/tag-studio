...
- **macOS**: Keychain
- **Linux**: Secret Service (systemd user session or GNOME Keyring)
...
**Installer Signing**: Releases are distributed as signed installers:
- **Windows**: Authenticode-signed `.exe` (NSIS)
- **macOS**: Notarized `.dmg` (Apple Developer ID)
- **Linux**: Published as `.AppImage`, `.deb`, and `.rpm` with GPG signatures
...
2. Verify the signature (Windows: check the publisher; macOS: check notarization; Linux: verify GPG signature)
...
2. Use OS-level encryption (BitLocker on Windows, FileVault on macOS, LUKS on Linux)
...
- On Windows & macOS, Tag Studio will notify you of updates; click "Install" to apply them
- On Linux, use your package manager to update
